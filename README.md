...
6. **Five roots, and AIRE never writes outside them**:
   ...
   - `tests/` — `unit/`, `behavior/` ( Gherkin step definitions), `e2e/` (Playwright), and the eval framework nested at `tests/.evals/` (`config.json`, `rubrics/`, `scripts/`, `behavior/`, `ci-manifest.d/` — one append-only fragment per work unit, merged with `config.json` at CI run time)
   - `spec/` — specs and docs only: `behavior.feature` at its root (once per cycle) plus five
     ...
     plans) — both written for every work unit at the design handoff, before any code — `playwright-specs/`
     (the Playwright Planner's Approved `<story-slug>.md` plan; the `.spec.ts` it drives stays in
     `tests/e2e/`) — and
     `context-project/` (human input). 🔴 The whole tree is removed by `archive-epic` at cycle close,
     once it is safely archived — every cycle rebuilds it from Atlas truth
...
```
context-project/
└── Abc.corp/            ← named exactly after the module
    └── interview.md     ← how Abc.corp works, where things live
```

**How it's used.** At workflow start AIRE asks: *"Are there any context-project artifacts I should use for this task?"*
- **Yes** → paste the exact file/folder path (e.g. `spec/context-project/existing-knowledge/Abc.corp/interview.md`). Only that path is read — used as background context when building `requirements.md` and the plan.
...
## Parallel Development Without Bookkeeping Conflicts

Several developers can build different stories of the same epic at once, each in their own clone, while the ve signs stories off on the epic branch. None of their PRs conflict on the framework's own files, because **every run that works on a single work unit keeps its own audit trail and state**:

- `runtime-artifacts/stories/story-1.2/audit.md` and `state.md` for Story 1.2, `stories/story-1.3/…` for Story 1.3, `stories/bug-PROJ-301/…`, `stories/ve-PROJ-102/…` for a ve test-plan run, and so on.
- The shared `runtime-artifacts/audit.md` and `aire-state.md` are written only by runs on the integration branch itself (planning and design, `ve-list-work` sign-offs, `archive-epic`).
- Every reader combines the shared files with the fragments. The newest record of a story wins, and `archive-epic` folds everything back into the shared files at cycle close.
- Before every commit, a work-unit run checks that it changed nothing under `runtime-artifacts/` except its own folder.

`.gitattributes` `merge=union` alone could not do this. It keeps both versions of an edited Story Tracker row (silent duplicates), and GitHub's merge button ignores it. Details: `aire-workflow/common/parallel-work-state.md`.

## The Evidence Every Story Leaves Behind

Every `dev-implement`, `bug-fix-implement` and `enhancement-implement` run commits the same evidence set in the same format, so reviewers, the Verification Engineer and CI can rely on it:

- `reports/unit-test-evidence/<key>/` — baseline and full regression logs, the unit test run, a `coverage/` folder (the tool's report plus `changed-lines-coverage.json`, the coverage of just the lines this story changed, which is what the 90% gate measures), and an `evidence-manifest.md`.
- `reports/behavior-test-evidence/<key>/b1/` and `b2/`, each with its run log, report and manifest. `b3/` exists **only on the last story of the cycle**, when the epic-wide tier runs.
- `reports/api-contract-test-evidence/<key>/` and `reports/playwright-test-evidence/<key>/`, only when the story touches an API or UI.
- `reports/eval-evidence/<key>/` — `eval.json` and `eval-summary.md` (the scorecard pasted into the PR), plus the static-check and judge outputs.
- `reports/reviews/<key>-code-review-v<X>.md` and `reports/code-security-reviews/<key>-security-review-v<X>.md`. Both are keyed by the story, so parallel stories never collide.

For a frontend story (its plan has a *Frontend Components Generation* step), Playwright is made ready before any code is written: the runner, the browsers and Playwright's official test agents are installed automatically. If the agents had to be installed, you are asked once to restart Claude Code, and the run then continues where it stopped. Any story that follows a UI story also installs the runner when needed and runs the full Playwright regression sweep.

A gate that did not apply gets no folder; its N/A and the reason are recorded in `eval.json` and `eval-summary.md`. Before every commit an **Artifact Completeness Check** confirms every required file exists, follows its set format and agrees with the scorecard. A missing piece of evidence is produced by re-running the gate that should have made it, never written by hand. Details: `aire-workflow/common/work-unit-artifacts.md`.

## When Self-Healing Runs Out — Fixing It Yourself

Every automatic fix loop gets 3 attempts. If a gate is still failing after that, the run stops at that gate and tells you, in plain words:

- **What is failing**: each failure with its file and line, and the measured result against the threshold.
- **Why it stopped**: the specific blocker it could not get past.
- **What it tried**: the three attempted fixes, which are left in your working tree for you to keep, adjust or revert.
- **How to fix it yourself**: the exact command to reproduce it, the evidence files to read, where the problem usually is for that gate, and the command that proves your fix.
- **What not to do**: the shortcuts that gate would reject, such as deleting or skipping a test, adding a suppression comment or lowering a threshold.

Then fix it and **type the same keyword again** (`dev-implement`, `bug-fix-implement`, `ticket-implement <ID>`, `epic-enhance`, `epic-bugfix` or `pr-fix <n>`). The framework recognises the stopped work and checks what you changed. It never undoes your edits. It re-runs every gate from the first code gate, and when everything passes it reviews, commits and raises the PR itself.

If a static-check tool can't be installed on your machine, you get the same kind of message: what was tried, the command to install the tool yourself, and how to hand the work back.

Details: `aire-workflow/common/self-heal-limit-guidance.md`.
...
### Step 8 — Publish to Atlas: `stitch-delta`
...
6. Once the verdict is clean: **`[BUG]` PR straight to the base branch** (via `pr-generator`), followed by an **AUTO `pr-review`** pass. The ticket's test plan was already written and committed on the bug branch at the design checkpoint, so the test docs reside in this same `[BUG]` PR.
7. **The ticket stays In Development** — it is never moved forward by this workflow. Once the fix is on the bug branch (the `[BUG]` PR still open), ve builds that branch locally, executes the ticket's manual test steps, then **invokes `ve-list-work`** Option B on the bug branch and answers one prompt — `<tracker key> approve` or `<tracker key> reject`. Approve → comment `ve approved the story` + `ve-approved` label, ticket → Ready for Testing; reject → comment `ve rejected the story` + `ve-rejected` label, ticket stays In Development (ve manually log the bug via `/raise-defect`). 
8. **MANUAL archive (bug mode)** — 🔴 **not automatic**. Once ve sign-off is done and all ve work is committed to the bug branch (a rejected ticket still archives — archive-epic shows a notice about the open PR), pull that branch and invoke **`/archive-epic`** yourself — it archives `spec/` + `reports/` + `runtime-artifacts/` into `aire-archives/bugs/<BUG-ID>-<name>/`, writes the cycle's RE delta to `spec/plans/delta/<BUG-ID>-<name>/`, and then removes the live `spec/`, `reports/` and `runtime-artifacts/` trees in full. It never stitches. It must run **before** the `[BUG]` PR merges (its commit resides in the open PR). **After that PR merges, run `/stitch-delta` on the base branch** — it reads the delta from the archive and publishes it to Atlas. The cycle is not closed until it does.
...
6. Once the verdict is clean: **`[ENH]` PR straight to the base branch** (via `pr-generator`), followed by an **AUTO `pr-review`** pass. The ticket's test plan was already written and committed on the enhancement branch at the design checkpoint, so the test docs reside in this same `[ENH]` PR.
7. **The ticket stays In Development** — once the changes are on the enhancement branch (the `[ENH]` PR still open), ve builds that branch locally, executes the ticket's manual test steps, then **invokes `ve-list-work`** Option B on the enhancement branch and answers one prompt — `<tracker key> approve` or `<tracker key> reject`. Approve → comment `ve approved the story` + `ve-approved` label and Ticket → Ready for Testing; reject → comment `ve rejected the story` + `ve-rejected` label, ticket stays In Development (ve manually log the bug via `/raise-defect`).
8. **MANUAL archive (enhancement mode)** — 🔴 **not automatic**. Once ve sign-off is done and all ve work is committed to the enhancement branch (a rejected ticket still archives — archive-epic shows a notice about the open PR), pull that branch and invoke **`/archive-epic`** yourself — it archives `spec/` + `reports/` + `runtime-artifacts/` into `aire-archives/enhancements/<ENH-ID>-<name>/`, writes the cycle's RE delta to `spec/plans/delta/<ENH-ID>-<name>/`, and then removes the live `spec/`, `reports/` and `runtime-artifacts/` trees in full. It never stitches. It must run **before** the `[ENH]` PR merges (its commit resides in the open PR). **After that PR merges, run `/stitch-delta` on the base branch** — it reads the delta from the archive and publishes it to Atlas. The cycle is not closed until it does.
...
All of your test documentation lives under `spec/test-plans/<TICKET-ID>-<title>/`, one folder per story — present on the integration branch from the design handoff onward, before any code is written.
...
2. The story's test plan is on that branch (from the design checkpoint; any open /ve-implement
   rework PR has merged)
...
`spec/playwright-specs/<story-slug>.md` (the Approved plan), and
...
1. The workflow reaches its design handoff and pushes the requirements, design artifacts and every story's test plan to the integration branch.
2. You pull that branch and review the test plans while the developer builds. Development and Test Plan proceed independently.
3. The developer's pull request for the story merges into the integration branch.
4. You run `ve-list-work` on the integration branch and pick Option A to confirm what has merged.
5. You build the system locally from the integration branch and execute the story's manual test steps.
6. You run `ve-list-work` again and pick Option B to approve or reject the story. Approved stories move to Ready for Testing; rejected items stay In Development and you manually log a defect with the `/raise-defect` skill.
7. Optionally, invoke `/playwright-implement <story>` to turn its UI-relevant manual cases into executable Playwright automation (Step 3 above). Frontend stories already have Playwright tests from `dev-implement`.
...
>
> Single session: `dev-implement` never forks itself or hands any step to a subagent — everything from story selection to the PR happens in the session you typed it in (the only exception is Playwright's own Planner/Generator/Healer agents inside the UI gate). If it detects it is running inside a fork, it stops before claiming the story.
...
### `epic-enhance` / `epic-bugfix <TICKET-ID | description>` — change or fix what the epic already built, mid-cycle

For a change that arrives while the epic is still open — new or changed behaviour, or a bug the ve found in an earlier story. Instead of a separate base-branch cycle, it becomes **one new change story on the epic branch**:

- **Impact analysis** maps the change to the files, the earlier stories that own them (from the manifest fragments, `[Story N.M]` commit history and `git blame`), and each affected acceptance criterion — classified unchanged, amended or superseded.
- **One document** — `spec/plans/change-story-<N.M>.md` (`bugfix-story-<N.M>.md` for bugs) — holds the change, the impact, the new story's ACs, the test impact (which scenarios, test-plan cases and tests of earlier stories change) and the plan. **Approving it is the only question in the whole run** — where a decision is needed, the framework makes it and lists it in the document for you to overrule with *Request Changes*.
- It then updates the earlier stories' ACs, `.feature` scenarios, test plans and unit/API/Playwright tests, and runs the **full `dev-implement` gate chain** plus a **Test Impact Reconciliation** check: only tests on the approved *Test Change Authorization* list may change — any other red test is a regression fixed in the code.
- `epic-bugfix` has its own flow: it reproduces the bug (a failing test) before writing the plan and again before fixing, and **automatically determines whether the defect is AI-generated** — every defective line is traced to the commit that introduced it and checked for the framework's AI markers; on positive evidence the bug gets the `ai-generated-defect` label and an `is caused by` link to the originating story, with no question asked.
- It raises a `[STORY]` PR into the epic branch. If the affected stories have no code yet, it only amends their specs. A story being built right now, or with an open PR, blocks the change (use `pr-fix` on that PR instead).

Full rules and edge cases: `aire-workflow/workflows/epic-enhance.md` and `aire-workflow/workflows/epic-bugfix.md`.

### `pr-fix [PR-NUMBER]` — fix a rejected story, bug or enhancement PR

When the developer or the ve rejects an open `[STORY]`, `[BUG]` or `[ENH]` PR — comments, a *Request changes* review, or a ve rejection on the tracker while the PR is still open — type `pr-fix`. It asks **which PR** (that is normally the only question), reads every unresolved human comment, sorts each into fix / answer / decline / defer, fixes the work unit on the PR's own branch, and **re-runs the complete gate chain of the workflow that built it** (unit + coverage, Gherkin B1–B3, API & contract, regression, D1–D7, Playwright, code review with security and J1/J2, CI preflight/attestation). It then pushes to the **same** PR, refreshes the eval scorecard, replies to each comment and re-requests review. It asks one more question only when a comment would change an approved acceptance criterion or two reviewers disagree. It never merges, approves, resolves threads or dismisses reviews, and — like `dev-implement` — runs entirely in your session, never in a fork. A PR that has already merged cannot be updated: log the finding with `/raise-defect` instead. Full rules and rejection scenarios: `aire-workflow/workflows/pr-fix.md`.
