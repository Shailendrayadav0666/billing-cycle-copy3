...
delta_diff D1_lint 'ruff check --output-format=concise . 2>/dev/null | sed -nE "s/^([^:]+):[0-9]+:[0-9]+: ([A-Z0-9]+) (.*)\$/\2\t\1\t\3/p"' "src/backend" "requirements.txt"
delta_diff D1_lint 'npx --yes oxlint --format=json . 2>/dev/null | jq -r ".diagnostics[]? | [.code, .filename, .message] | @tsv"' "src/frontend" "package.json"

delta_diff D2_types 'mypy . 2>/dev/null | sed -nE "s/^([^:]+):[0-9]+: error: (.*)  \[([a-zA-Z0-9_-]+)\]\$/\3\t\1\t\2/p"' "src/backend" "requirements.txt"
record_multi_root D2_types "src/frontend" N/A "plain JavaScript (.jsx), no tsconfig.json in this root — no type checker in the ecosystem for untyped JS (eval-framework.md Section 2.5.1 Reason 1)"

delta_diff D4_deps 'pip-audit -r requirements.txt -f json 2>/dev/null | jq -r ".dependencies[]? | select(.vulns and (.vulns|length>0)) | .name as \$n | .vulns[] | [.id, \$n, .description] | @tsv"' "src/backend" "requirements.txt"
delta_diff D4_deps 'npm audit --json 2>/dev/null | jq -r ".vulnerabilities // {} | to_entries[] | [.value.severity, .key, ((.value.via // []) | map(if type==\"object\" then .title else . end) | join(\"; \"))] | @tsv"' "src/frontend" "package.json"

delta_diff D5_licenses 'pip-licenses --format=json 2>/dev/null | jq -r --arg disallowed "$AIRE_DISALLOWED_LICENSES" "(\$disallowed | split(\",\")) as \$bad | .[] | select(.License as \$l | \$bad | index(\$l)) | [\"license\", .Name, (.License + \" is disallowed\")] | @tsv"' "src/backend" "requirements.txt"
delta_diff D5_licenses 'npx --yes license-checker@25.0.1 --json 2>/dev/null | jq -r --arg disallowed "$AIRE_DISALLOWED_LICENSES" "(\$disallowed | split(\",\")) as \$bad | to_entries[]? | (.value.licenses // \"\") as \$lic | (if (\$lic|type)==\"array\" then \$lic else [\$lic] end) as \$lics | select((\$lics - (\$lics - \$bad)) | length > 0) | [\"license\", .key, ((\$lics|join(\",\")) + \" is disallowed\")] | @tsv"' "src/frontend" "package.json"

delta_diff D6_complexity 'radon cc -j . 2>/dev/null | jq -r --argjson max "${AIRE_MAX_CYCLOMATIC_COMPLEXITY:-12}" "to_entries[] | .key as \$f | .value[] | select(.complexity > \$max) | [\"complexity\", \$f, (.name + \" complexity=\" + (.complexity|tostring))] | @tsv"' "src/backend" "requirements.txt"
record_multi_root D6_complexity "src/frontend" ERROR "no complexity tool resolved for this root's linter — oxlint (this project's configured linter) has no complexity rule. This is an honest ERROR per eval-framework.md Section 2.5.1 (a missing tool is incomplete setup, never an earned N/A), not a deferred-setup N/A. Resolution: add eslint's complexity rule (or an equivalent JS complexity tool) to this root, or remove D6_complexity from ci.gates deliberately."
