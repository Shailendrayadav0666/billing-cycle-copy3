...
delta_diff D6_complexity 'radon cc -i "venv" -j . 2>/dev/null | jq -r --argjson max "${AIRE_MAX_CYCLOMATIC_COMPLEXITY:-12}" "to_entries[] | .key as \$f | .value[] | select(.complexity > \$max) | [\"complexity\", \$f, (.name + \" complexity=\" + (.complexity|tostring))] | @tsv"' "src/backend" "requirements.txt"
delta_diff D6_complexity 'npx --yes eslint@8 --no-eslintrc -c "$(git rev-parse --show-toplevel)/tests/.evals/eslint-complexity.config.json" --ext .js,.jsx --rule "{\"complexity\":[\"error\", ${AIRE_MAX_CYCLOMATIC_COMPLEXITY:-12}]}" --format json src 2>/dev/null | jq -r --arg cwd "$(pwd)" ".[] | (.filePath | if startswith(\$cwd+\"/\") then .[(\$cwd|length)+1:] else . end) as \$f | .messages[]? | select(.ruleId==\"complexity\") | [\"complexity\", \$f, .message] | @tsv"' "src/frontend" "package.json"
