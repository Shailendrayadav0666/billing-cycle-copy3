...
delta_diff D2_types 'mypy --no-error-summary --output=json . 2>/dev/null | jq -r ". | [(.code // \"mypy\"), .file, .message] | @tsv"' "src/backend" "requirements.txt"
delta_diff D4_deps 'pip-audit --format json -r requirements.txt 2>/dev/null | jq -r ".dependencies[]? | .name as \$n | .version as \$v | .vulns[]? | [.id, \"requirements.txt\", (\$n + \"==\" + \$v + \": \" + .description)] | @tsv"' "src/backend" "requirements.txt"
delta_diff D5_licenses 'pip-licenses --format json 2>/dev/null | jq -r --arg bad "$AIRE_DISALLOWED_LICENSES" "def badlist: (\$bad | split(\",\")); .[] | select(.License as \$l | badlist | index(\$l)) | [\"license\", .Name, (.Name + \" uses disallowed license \" + .License)] | @tsv"' "src/backend" "requirements.txt"
delta_diff D6_complexity 'radon cc -j . 2>/dev/null | jq -r --arg max "$AIRE_MAX_CYCLOMATIC_COMPLEXITY" "to_entries[] | .key as \$file | .value[] | select(.complexity > (\$max|tonumber)) | [\"complexity\", \$file, (.name + \" has cyclomatic complexity \" + (.complexity|tostring) + \" > \" + \$max)] | @tsv"' "src/backend" "requirements.txt"
delta_diff D1_lint 'npx --yes oxlint --format json src 2>/dev/null | jq -r ".diagnostics[]? // .[]? | [(.code // .ruleId // \"oxlint\"), (.filename // .file // \"src\"), .message] | @tsv"' "src/frontend" "package.json"
record_multi_root D2_types "src/frontend" N/A "plain JSX — no TypeScript/type checker configured for this stack (tests/.evals/config.json ci.roots[]._rootsComment)"
delta_diff D4_deps 'npm audit --json --audit-level=high 2>/dev/null | jq -r ".vulnerabilities // {} | to_entries[]? | [.value.severity, \"package.json\", (.key + \": \" + .value.severity + \" severity vulnerability\")] | @tsv"' "src/frontend" "package.json"
delta_diff D5_licenses 'npx --yes license-checker --json 2>/dev/null | jq -r --arg bad "$AIRE_DISALLOWED_LICENSES" "def badlist: (\$bad | split(\",\")); to_entries[] | select(.value.licenses as \$l | badlist | index(\$l)) | [\"license\", .key, (.key + \" uses disallowed license \" + .value.licenses)] | @tsv"' "src/frontend" "package.json"
delta_diff D6_complexity 'npx --yes eslint --no-eslintrc -c eslint.config.js --format json src 2>/dev/null | jq -r ".[] | .filePath as \$f | .messages[]? | select(.ruleId==\"complexity\") | [\"complexity\", \$f, .message] | @tsv"' "src/frontend" "package.json"
