...
#   -p/--print            headless, non-interactive — required, there is no TTY on a GH Actions runner
#   --permission-mode acceptEdits   auto-approve file edit tool calls without a human in the loop;
#                                    deliberately NOT --dangerously-skip-permissions/bypassPermissions —
#                                    the CLI's own help text recommends those "only for sandboxes with
#                                    no internet access", which a GitHub-hosted runner is not
#   --allowedTools "Bash,Edit,Write,Read,Glob,Grep"   scoped to editing the repo and running the
#                                    project's own test/lint commands (Section 6.0's "scope the
#                                    permissions" rule) — no network-write or credential tools granted
#                                    beyond what the job's own env already holds
printf '%s' "$BRIEF" | claude -p --permission-mode acceptEdits --allowedTools "Bash,Edit,Write,Read,Glob,Grep" || report_and_exit "the repair CLI invocation failed on attempt ${attempt}." 1
