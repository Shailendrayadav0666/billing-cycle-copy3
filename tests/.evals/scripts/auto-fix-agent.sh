...
# flags resolved from `claude --help`, CLI v2.1.273 (resolved 2026-09-16T10:53:19Z):
#   -p/--print              headless mode, no interactive session
#   --dangerously-skip-permissions   no human present to approve tool calls on this ephemeral runner
#   --model claude-sonnet-5 pinned to match tests/.evals/config.json judge.model, for a consistent model across repair and judge
#   --allowedTools           scoped to reading/editing this repo and running its own commands - no network-write/credential tools beyond what the job already holds
printf '%s' "$BRIEF" | claude -p --dangerously-skip-permissions --model claude-sonnet-5 --allowedTools "Read Edit Write Bash Grep Glob" || report_and_exit "the repair CLI invocation failed on attempt ${attempt}." 1
