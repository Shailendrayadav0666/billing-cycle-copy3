...
${diff}"
  ...
  #   --model claude-sonnet-5 pinned to match tests/.evals/config.json judge.model
  #   --allowedTools "Read"   read-only scoring - the diff and rubric are already embedded in the prompt; Read is allowed only in case a citation needs the surrounding file
  resp="$(printf '%s' "$prompt" | claude -p --dangerously-skip-permissions --model claude-sonnet-5 --allowedTools "Read" 2>"$claude_err_file")"; rc=$?
