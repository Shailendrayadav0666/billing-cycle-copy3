...
${diff}"
  ...
  # flags resolved from `claude --help`, CLI v2.1.268 (2026-09-11):
  #   -p/--print          headless, non-interactive — no TTY on a GH Actions runner
  #   --model "$MODEL"    pinned to tests/.evals/config.json judge.model — never the CLI's own default
  #   --permission-mode acceptEdits   the judge only reads (Read/Glob/Grep/git diff/show/log — no Edit/
  #                                    Write/Bash(*) granted), so this never actually triggers an edit
  #                                    approval; kept consistent with auto-fix-agent.sh rather than the
  #                                    riskier bypassPermissions
  #   --allowedTools "Read,Glob,Grep,Bash(git diff*),Bash(git show*),Bash(git log*)"   read-only scope —
  #                                    the judge scores a diff already in $prompt; it never needs to edit
  #   deliberately NOT --output-format json: the prompt itself asks Claude to return strict JSON as its
  #   text response, and the sed/jq extraction below expects raw text, not the CLI's own JSON envelope
  resp="$(printf '%s' "$prompt" | claude -p --model "$MODEL" --permission-mode acceptEdits --allowedTools "Read,Glob,Grep,Bash(git diff*),Bash(git show*),Bash(git log*)" 2>"$claude_err_file")"; rc=$?
  ...
  if [ -z "$json" ]; then
    resp="$(printf '%s' "$prompt" | claude -p --model "$MODEL" --permission-mode acceptEdits --allowedTools "Read,Glob,Grep,Bash(git diff*),Bash(git show*),Bash(git log*)" 2>"$claude_err_file")" || true
    json="$(printf '%s' "$resp" | sed -n '/{/,/}/p' | jq -c '.' 2>/dev/null || true)"
  fi
  rm -f "$claude_err_file"
  if [ -z "$json" ]; then
