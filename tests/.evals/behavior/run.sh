...
run_features() { # $@ = feature files
  ...
  # Python / pytest-bdd (common/behavior-spec.md Section 4.1): each spec/behavior/<slug>.feature has a
  # 1:1 test module tests/behavior/test_<slug-with-dashes-and-dots-as-underscores>.py that calls
  # scenarios("../../spec/behavior/<slug>.feature"). Resolve each feature file to its module and run
  # pytest against exactly those modules — never the whole tests/behavior/ tree, so b1 cannot
  # accidentally execute a neighbouring unit's contract.
  local modules=()
  local f base mod
  for f in "$@"; do
    ...
    if [ ! -f "$mod" ]; then
      echo "run.sh: no pytest-bdd module found for feature '$f' (expected $mod)" >&2
      return 2
    fi
    modules+=("$mod")
  done
