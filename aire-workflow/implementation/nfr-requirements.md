...
## Prerequisites
- Functional Design must be complete (artifacts in `spec/plans/functional-design.md`); if it was skipped, use the intake brief + requirements + stories as inputs instead
...
- Read functional design artifacts from `spec/plans/functional-design.md` (if it exists; else use the intake brief + requirements + stories)
...
### Step 4: Store Plan
- Save as `spec/spec-generation/nfr-requirements-generation.md` (NFR Design keeps its own plan file, so neither overwrites the other)
...
- Create `spec/plans/nfr.md` with a `## NFR Requirements` section (NFR Design later appends its own `## NFR Design` section to the same file)
...
```markdown
> ** <u>**REVIEW REQUIRED:**</u>**  
> Please examine the NFR requirements at: `spec/plans/nfr.md`
...
- Mark NFR Requirements stage complete in runtime-artifacts/aire-state.md
