...
## Story statuses
The only Story Tracker statuses are **Ready for Development**, **In Development** and **Ready for Testing**, written as plain text in state files, tracker comments and every message (no emojis). A state file from an earlier framework version may still carry an emoji before the status (for example `🔵 In Development`); readers treat it as the same status and writers rewrite it in plain text the next time they update that row.
