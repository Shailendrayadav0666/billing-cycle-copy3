...
**Exceptions (CLAUDE.md "Question Format")**: Tracker Selection, Context Opt-In and the CI/CD Opt-In are asked in chat only, and the CI/SonarQube setup gate is emitted verbatim as plain text. Workflow-internal one-line prompts that a rule file defines inline (for example `yes / skip`) are also asked in chat.
...
```markdown
Create spec/{phase-name}-questions.md with all questions
```
...
```
Read spec/{phase-name}-questions.md
Extract all answers
