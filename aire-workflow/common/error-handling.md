...
5. **Document**: Log the error and resolution in `runtime-artifacts/audit.md`
...
**Error**: Existing `runtime-artifacts/aire-state.md` is corrupted
...
- **Workaround**: Proceed without tests, mark for manual completion and track as a follow-up
...
**Scenario**: `runtime-artifacts/aire-state.md` is corrupted or inconsistent

**Recovery Steps**:
1. Create backup: `runtime-artifacts/aire-state.md.backup`
...
5. Document the gap in `runtime-artifacts/audit.md`
...
3. Reset stage status in `runtime-artifacts/aire-state.md`
...
2. Document skip reason in `runtime-artifacts/audit.md`
3. Mark stage as "SKIPPED" in `runtime-artifacts/aire-state.md`
...
- **Solution**: 
  ...
  2. Check if stage was marked complete in runtime-artifacts/aire-state.md
...
**Error**: runtime-artifacts/aire-state.md shows stage complete but artifacts don't exist
- **Cause**: State file updated but artifact generation failed
- **Solution**:
  1. Mark stage as incomplete in runtime-artifacts/aire-state.md
...
**Error**: Artifacts exist but runtime-artifacts/aire-state.md shows stage incomplete
- **Cause**: Artifact generation succeeded but state update failed
- **Solution**:
  1. Verify artifacts are complete and valid
  2. Update runtime-artifacts/aire-state.md to mark stage complete
  3. Proceed to next stage
- **Recovery**: Update state file to reflect actual completion

**Error**: Multiple stages marked as "current" in runtime-artifacts/aire-state.md
- **Cause**: State file corruption, manual editing
- **Solution**:
  1. Review artifacts to determine actual progress
  2. Ask user which stage they're actually on
  3. Correct runtime-artifacts/aire-state.md to show single current stage
...
1. **Always validate state**: Check runtime-artifacts/aire-state.md matches actual artifacts
...
6. **Document recovery**: Log all recovery actions in runtime-artifacts/audit.md
...
5. **Document Everything**: Log all decisions and changes in `runtime-artifacts/audit.md`
